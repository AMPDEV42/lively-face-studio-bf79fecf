 ...
 * - Usage pesan bulan ini dengan progress bar
 * - Sisa top-up jika ada (untuk Pro)
 * - Frozen top-up notice (untuk non-Pro dengan sisa top-up)
 * - CTA upgrade/top-up yang kontekstual
 */

import { Link } from 'react-router-dom';
import { Crown, Sparkles, BarChart2, Zap, Lock, AlertTriangle } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { usePlan } from '@/hooks/usePlan';
import { formatNumber, getProgressColor } from '@/lib/plan-config';
...
export default function PlanStatusBanner({ className = '' }: PlanStatusBannerProps) {
  const { planConfig, isPro, stats, messagePercent } = usePlan();
      ...
      >
        <div className="flex items-center gap-3">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-white/35">
                  {formatNumber(stats.messagesThisMonth)} / {formatNumber(stats.totalMessageQuota ?? limits.messagesPerMonth)} pesan
                  {hasActiveTopUp && (
                    <span className="text-violet-400 ml-1">(+{stats.topUpMessages.toLocaleString('id-ID')} top-up)</span>
                  )}
                </span>
          ...
          <div className="flex flex-col gap-1.5 shrink-0">
            ...
            ) : isAtLimit && stats.topUpHeadroom > 0 ? (
              <Link to="/pricing#topup">
                <Button
                  size="sm"
                  className="h-7 w-full text-[10px] gap-1 border-0 px-2 bg-amber-600/30 hover:bg-amber-600/40 text-amber-300"
                >
      ...
      {hasFrozenTopUp && (
        ...
        >
          ...
          <p className="text-white/40 flex-1">
            <span className="text-amber-300 font-medium">{stats.topUpMessages.toLocaleString('id-ID')} pesan</span> top-up dibekukan
          </p>
